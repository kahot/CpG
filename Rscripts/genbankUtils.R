...
accCodesByRulesFullLength <- function(virusName, virusCode=NA, segmentName=NA, humanOnly=F, minLength, maxLength, isHIV=F) {
  ...
  if(isHIV == T) {
    #only this line is HIV specific, we added "Subtype B" as a tag
    nameSection <- paste0("(", virusName, "%5BOrganism%5D+OR+", virusName, "%5BAll+Fields%5D)+AND+Subtype+B%5BAll+Fields%5D+AND+Patient+Code%5BAll+Fields%5D")
  } else {
    nameSection <- paste0("(", virusName, "%5BOrganism%5D+OR+", virusName, "%5BAll+Fields%5D)")
  }
...
genbankMetadata <- function(accCodes, isHIV=F) {
  ...
  if(length(accCodes) > 500) {     #chopUp
    ...
    for(k in 1:numChunks) {
      ...
      if(isHIV==F) {
        ...
        if(isHIV==T) {
          metadata <- rbind(metadata, genbankQueryFullHIV(accList, accCodeChunk))
        }
      }
    }
    ...
    if(isHIV==F){
      ...
      if(isHIV==T) {
        metadata <- genbankQueryFullHIV(accList, accCodes)  
      }
    }
  }  
...
genbankQueryFullHIV <- function(accList, accCodeChunk){
  ...
  for(i in 1:length(accCodeChunk)) {
    ...
    gene <- sub(".*/[Gg]ene[[:blank:]]*[=:][[:blank:]]*","", gene)
    gene <- gsub("\"","", gene)
    gene <- sub(":.*","", gene)
    gene <- sub("/gene=", "", gene)
    ...
    ## get patient code ##
    patientCodeValue <- entry[grep("Patient Code", entry, ignore.case=TRUE)]
    patientCodeValue <- sub(".*[Pp]atient[[:blank:]][Cc]ode[[:blank:]]*::[[:blank:]]", "", patientCodeValue)
    if(length(patientCodeValue)==0) {
      patientCode[i] <- NA
    } else {
      patientCode[i] <- patientCodeValue
    }
  ...
  result <- cbind(accCodeChunk, virusName, seqLength, collectionDate, location, gene, note1, note2, patientCode)
